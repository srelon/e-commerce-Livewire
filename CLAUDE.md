...
**`ui/base/SortSelect.vue`** — the compact "Sort by..." dropdown, extracted out of `ProductList.vue` (`options: {value, label}[]` prop instead of hardcoded `<option>` tags) since News will want the same compact sort control with its own different option list (newest/oldest/most popular, not price-related). **Not the same component as `ui/base/BaseSelect.vue`** — `BaseSelect` is a labeled form field (validation error slot, `form-field-*` mixins, meant for actual forms like `ContactForm.vue`) and takes its options via a `<slot>`; `SortSelect` is unlabeled, takes a plain `options` array, and keeps its own compact standalone styling — don't merge the two just because they're both "a select."

...
- **The admin panel (`/admin`) also connects directly to a public channel** (`reviews.products.{slug}`, live-refreshing the product-edit page's Reviews tab) — no ticket needed since it's public, not private. See `backend/app/Livewire/CLAUDE.md` § Real-time for the Alpine/Livewire wiring; its WS URL comes from `config('websocket.url')` (`WS_URL` env in `backend/.env`, independent of the frontend's `VITE_WS_URL`).

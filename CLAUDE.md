...
Plain Livewire admin at `/admin` — every resource/page (Users, Roles + permission matrix, Admins, Menus tree, Dashboard) uses **Livewire PowerGrid** for tables and **Flux UI (Free tier)** for form components. This replaced an earlier FilamentPHP admin panel — the migration (started 2026-08-07, cut over 2026-08-10: `filament/filament` + `solution-forest/filament-tree` removed, `app/Filament/` deleted, `/admin-lw` renamed to `/admin`) is complete; there is no Filament in this project anymore.
...
**Menu tree requires a manual "Save changes" click** (`isDirty` flag gates a `flux:button`/Cancel pair — see `Menu\Tree` / `menu/tree.blade.php`) rather than auto-saving on drop. This is intentional, not an oversight — a client-side "auto-save on drop" was tried repeatedly against the old Filament tree package and abandoned after several attempts kept racing the vendor drag library's own DOM mutations (duplicate saves, saves firing mid-drag). Don't re-attempt auto-save on the Livewire tree without the user explicitly asking again.
